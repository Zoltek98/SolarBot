...
public class Giove {
     double N, i, w, a, e, M, L, E0, E1, x, y, r, v, ecl, lon, x2, y2, z, d, sup, ar, dec, AO, xorr, yorr, zorr, xorr2, xggio, yggio, zggio;
        double xeclip, yeclip, zeclip, longio, latgio, temposid, azimuth, altitude;
        double pi = 3.14159265358979323846;
        Date adesso;
        double sup1, sup2, sup3, sup4, sup5,sup6,sup7;//variabile da usare altrimenti senza nome
    private double latitudine, longitudine, LHA, LST, GMST0, UTt, sinH, h, hAlba, UT_Sole_al_sud, cosLHA, hTramonto;
        public Giove(double d, double ecl, Date ora)
        {
            this.ecl = ecl;
            this.d = d;
            adesso = ora;
            N = Sole.rev(100.4542 + 2.76854E-5 * d);
            i = Sole.rev(1.3030 - 1.557E-7 * d);
            w = Sole.rev(273.8777 + 1.64505E-5 * d);
            a = 5.20256;
            e = Sole.rev(0.048498 + 4.469E-9 * d);
            M =  Sole.rev(19.8950 + 0.0830853001 * d);
        }
        public void Posizione(double latitudine, double longitudine)
        {
            ...
            longio= Math.atan2(yeclip, xeclip) * 180 / pi;
            latgio = Math.atan2(zeclip, Math.sqrt(xeclip * xeclip + yeclip * yeclip)) * 180 / pi;
            latgio = Sole.rev(latgio);
            ...
            double Ms = saturno.getM();
             ...
             xggio = sole.getXequat() + xeclip;
             yggio = sole.getYequat() + yeclip;
             zggio = sole.getZequat() + zeclip;
             ar = Sole.rev(Math.atan2(yggio, xggio) * 180 / pi);
             dec = Math.atan2(zggio, Math.sqrt(xggio * xggio + yggio * yggio)) * 180 / pi;
    ...
    public String getTramonto() {
        ...
        GMST0 = L + 180;
        L = M + w;
        UTt = Double.parseDouble((adesso.getHours() - 1) + "." + (adesso.getMinutes() * 5.0 / 3));
        LST = GMST0 + UTt * 15.04107 + longitudine;
        LHA = LST - ar;
        sinH = Math.sin(latitudine * pi / 180) * Math.sin(dec * pi / 180) * Math.cos(latitudine * pi / 180) * Math.cos(dec * pi / 180) * Math.cos(LHA * pi / 180);
