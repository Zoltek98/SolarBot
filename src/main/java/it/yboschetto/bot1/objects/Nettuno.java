 ...
 */
public class Nettuno {
    double N, i, w, a, e, M, L, E0, E1, x, y, r, v, ecl, lon, x2, y2, z, d, sup, ar, dec, AO, xorr, yorr, zorr, xorr2, xgura, ygnet, zgnet;
    double xeclip, yeclip, zeclip, lonnet, latnet, temposid, azimuth, altitude;
    double pi = 3.14159265358979323846;
    Date adesso;
    private double latitudine, longitudine, LHA, LST, GMST0, UTt, sinH, h, hAlba, UT_Sole_al_sud, cosLHA, hTramonto;
    public Nettuno(double d,double ecl,Date ora)
    {
    ...
    public void Posizione(double latitudine, double longitudine)
    {
        ...
        lonnet= Math.atan2(yeclip, xeclip) * 180 / pi;
        latnet = Math.atan2(zeclip, Math.sqrt(xeclip * xeclip + yeclip * yeclip)) * 180 / pi;
        latnet = Sole.rev(latnet);
        ...
        ygnet = sole.getYequat() + yeclip;
        zgnet = sole.getZequat() + zeclip;
        ar = Sole.rev(Math.atan2(ygnet, ygnet) * 180 / pi);
        dec = Math.atan2(zgnet, Math.sqrt(xgura * xgura + ygnet * ygnet)) * 180 / pi;
